...
const JobLoopItem = ({ job, className }) => {
  return (
    <div className={` ${className} card relative py-5 mb:py-[15.75px] 2xl:py-[33.75px] 3xl:py-[44.74px] 4xl:py-[60px] rounded-3xl md:rounded-4xl border-white border-2 shadow-(--ad-box-shadow) text-center flex flex-col justify-between items-center  gap-4 2xl:gap-8 3xl:gap-8 4xl:gap-10`}>
      ...
      <div className='mt-2.5 2xl:mt-4 3xl:mt-5.5 4xl:mt-7 px-[9.86px] md:px-[12.38px] 2xl:px-[16.38px] 3xl:px-[21.71px] 4xl:px-[29.22px] w-full'>
        <div className="font-inter font-bold hover:text-brand-yellow leading-[100%] name">{job.agency}</div>
        <div className="text-white font-inter font-bold hover:text-brand-yellow lowercase title">{job.title}</div>
        ...
        <div className="font-inter font-bold hover:text-brand-yellow location">{job.location}</div>
      </div>
    </div>
  );
};
