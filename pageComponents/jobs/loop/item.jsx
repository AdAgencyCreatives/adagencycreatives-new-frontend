...
const JobLoopItem = ({ job }) => {
  return (
    <div className="relative p-2 md:px-4 md:py-6 4xl:px-8 4xl:py-20 rounded-4xl border-white border-2 shadow-(--ad-box-shadow) text-center flex flex-col justify-between items-center gap-10">
      ...
      <div>
        <div className="font-alta text-base md:text-[30px] 3xl:text-[38px] 4xl:text-[50px] hover:text-yellow-400">{job.agency}</div>
        <div className="text-white font-semibold text-[10px] md:text-[20px] 3xl:text-[20px] 4xl:text-[25.21px] hover:text-yellow-400 lowercase">{job.title}</div>
        <div className="my-3 border-b-2 w-[60%] mx-auto" />
        <div className="font-alta font-bold text-[10px] md:text-[16px] 3xl:text-[18px] 4xl:text-[22.97px] hover:text-yellow-400">{job.location}</div>
      </div>
    </div>
  );
};
